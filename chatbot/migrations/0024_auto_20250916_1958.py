# Generated by Django 4.2.7 on 2025-09-16 19:58
...
def update_menu_items(apps, schema_editor):
    ...
    # Update chat item URL to use namespaced URL
    chat_item = SidebarMenuItem.objects.filter(url_name='chat').first()
    if chat_item:
        chat_item.url_name = 'chat:chat'
        chat_item.save()
    
    # Define updated menu items (removed dashboard, updated order)
    updated_items = [
        {
            'name': 'خانه',
            'url_name': 'home',
...
def reverse_menu_items_update(apps, schema_editor):
    ...
    # Remove added items
    url_names = ['home', 'financial_transactions', 'profile']
